...
| **T1-A.2: stress-test H-ENC-01 on a protocol with unseen val media** | T1-A on `multi_org_balanced` produced a tied outcome because val_seen_rate=1.0 at the media-name level — every val medium is in train, so the media_id encoder never falls back to UNK. The S1-predicted advantage of decomposed chemistry (generalizing across organism-unique medium names) is not exercised. Re-run the same T1-A comparison on `largest_by_rows` (val_seen_rate=0%) or a similar high-unseen-media protocol. | Always — this is the proper test of H-ENC-01 and it is mandatory before H-ENC-01 can be declared falsified. Filed 2026-05-12 in T1-DEC-001. |
