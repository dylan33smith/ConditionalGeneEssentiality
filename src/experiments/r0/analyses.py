...
def load_fitness(cols: Iterable[str] | None = None) -> pd.DataFrame:
    """Load canonical long-form fitness with the columns R0 needs.

    Applies one normalization step at load: case/whitespace folding on
    identifier columns (`expDesc`, `media`, `expGroup`). See
    `_normalize_string_keys` for rationale.
    """
