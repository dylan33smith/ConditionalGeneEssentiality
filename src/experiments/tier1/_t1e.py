"""T1-E — Decomposition-Mode Indicator Test (Hypothesis H-ENC-05).

Compares:
  A1 (chemistry_only):           425-dim binary multihot (T1-DEC-004 winner)
  A2 (chemistry_plus_mode_flags): 425-dim multihot + 4 mode-proportion features

Mode features: for each experiment, the fraction of its chemistry slots whose
dominant_mode (from representation_mode_per_canonical.parquet) is each of:
  - physical  (salt-form, defined media components)
  - mix       (mixed-source components)
  - extract   (yeast extract, tryptone — decomposition-inferred)
  - stressor  (applied stressors, never from workbook medium decomposition)

in_silico is omitted (zero in all data). The 4 proportions sum to 1.0 per
experiment, so they're linearly dependent — but the model can learn to
weight them independently, and dropping one would be an arbitrary choice
that could confuse interpretation.

Total condition dim:
  chemistry_only:           425
  chemistry_plus_mode_flags: 425 + 4 = 429
...
ARM_CHEM_MODE = "chemistry_plus_mode_flags"

MODE_COLS = ["physical", "mix", "extract", "stressor"]


def _build_mode_matrix(
    *,
    chemistry_path: Path,
    canonical_mode_path: Path,
    exp_to_row: dict[str, int],
) -> tuple[np.ndarray, dict]:
    """Build a dense (n_experiments, 4) mode-proportion matrix.

    For each experiment, computes the fraction of its chemistry slots
    falling into each dominant_mode category.
    """
    chem_df = pd.read_parquet(chemistry_path)
    canon_df = pd.read_parquet(canonical_mode_path)

    merged = chem_df.merge(
        canon_df[["canonical_id", "dominant_mode"]], on="canonical_id", how="left"
    )
    unmatched = int(merged["dominant_mode"].isna().sum())

    mode_counts = (
        merged.groupby(["experiment_id", "dominant_mode"])
        .size()
        .unstack(fill_value=0)
    )
    mode_props = mode_counts.div(mode_counts.sum(axis=1), axis=0)

    # Ensure all 4 mode columns exist
    for col in MODE_COLS:
        if col not in mode_props.columns:
            mode_props[col] = 0.0
    mode_props = mode_props[MODE_COLS]

    n_exp = max(exp_to_row.values()) + 1
    mat = np.zeros((n_exp, len(MODE_COLS)), dtype=np.float32)

    mapped = 0
    for eid, row_idx in exp_to_row.items():
        if eid in mode_props.index:
            mat[row_idx] = mode_props.loc[eid, MODE_COLS].to_numpy(dtype=np.float32)
            mapped += 1

    summary = {
        "mode_dim": len(MODE_COLS),
        "mode_cols": MODE_COLS,
        "n_experiments_mapped": mapped,
        "n_experiments_total": len(exp_to_row),
        "unmatched_canonical_ids": unmatched,
        "mean_proportions": {c: float(mat[:, i].mean()) for i, c in enumerate(MODE_COLS)},
    }
    return mat, summary
...
def _plot_figures(figures_dir: Path, metrics_df: pd.DataFrame, summaries_df: pd.DataFrame) -> None:
    ...
    arm_names = [ARM_CHEM_ONLY, ARM_CHEM_MODE]
    ...
    for col, ax, title in [
        ...
        x = np.arange(len(agg))
        ax.bar(x, agg["mean"], yerr=agg["std"], capsize=6, color=["C0", "C2"])
    ...
    colors = {ARM_CHEM_ONLY: "C0", ARM_CHEM_MODE: "C2"}
...
def run_t1e(cfg, *,
            ...
            output_root: str = "t1e",
            figures_dirname: str = "tier1_e",
            experiment_id_label: str = "T1-E_decomposition_mode",
            title: str = "T1-E Decomposition-Mode Test (H-ENC-05)") -> dict:
    """Run T1-E end-to-end. Returns the comparison payload."""
    ...
    chemistry_path = artifact_root / feature_contract["experiment_chemistry_table"]["path"]
    canonical_mode_path = artifact_root / "representation_mode_per_canonical.parquet"
    ...
    # Arm 2: chemistry + mode proportions
    mode_mat, mode_summary = _build_mode_matrix(
        chemistry_path=chemistry_path,
        canonical_mode_path=canonical_mode_path,
        exp_to_row=inputs.exp_to_row,
    )
    log.info("    mode shape=%s  cols=%s", mode_mat.shape, mode_summary["mode_cols"])
    log.info("    mapped %d/%d experiments, unmatched canonicals=%d",
             mode_summary["n_experiments_mapped"],
             mode_summary["n_experiments_total"],
             mode_summary["unmatched_canonical_ids"])
    log.info("    mean proportions: %s", mode_summary["mean_proportions"])

    chem_plus_mode = np.concatenate([chem_only, mode_mat], axis=1)
    log.info("    chemistry_plus_mode_flags shape=%s", chem_plus_mode.shape)

    condition_matrices = {
        ARM_CHEM_ONLY: chem_only,
        ARM_CHEM_MODE: chem_plus_mode,
    }
    ...
    for arm in [ARM_CHEM_ONLY, ARM_CHEM_MODE]:
    ...
    arm_metrics["arm"] = pd.Categorical(
        arm_metrics["arm"], categories=[ARM_CHEM_ONLY, ARM_CHEM_MODE], ordered=True
    )
    ...
    rmse_mode = float(arm_metrics.set_index("arm").loc[ARM_CHEM_MODE, "rmse_mean"])
    mae_chem = float(arm_metrics.set_index("arm").loc[ARM_CHEM_ONLY, "mae_mean"])
    mae_mode = float(arm_metrics.set_index("arm").loc[ARM_CHEM_MODE, "mae_mean"])

    rmse_gap = rmse_chem - rmse_mode
    mae_gap = mae_chem - mae_mode

    mode_clearly_better = (rmse_gap > threshold_rmse) and (mae_gap > threshold_mae)
    chem_clearly_better = (-rmse_gap > threshold_rmse) and (-mae_gap > threshold_mae)

    if mode_clearly_better:
        decision = "promote_chemistry_plus_mode_flags"
    ...
    comparison = {
        "rmse_gap_chem_minus_mode": float(rmse_gap),
        "mae_gap_chem_minus_mode": float(mae_gap),
        "rmse_threshold_locked_s2": threshold_rmse,
        "mae_threshold_locked_s2": threshold_mae,
        "mode_clearly_better": bool(mode_clearly_better),
        "chem_clearly_better": bool(chem_clearly_better),
        "decision": decision,
    }
    log.info("Decision: %s | RMSE gap (chem - mode) = %.4f (thr=%.4f) | MAE gap = %.4f (thr=%.4f)",
    ...
    metrics_df.to_parquet(output_dir / "t1e_metrics.parquet", index=False)
    summaries_df.to_parquet(output_dir / "t1e_summaries.parquet", index=False)
    payload = {
        ...
        "mode_summary": _to_jsonable(mode_summary),
        "condition_dims": {
            ARM_CHEM_ONLY: int(chem_only.shape[1]),
            ARM_CHEM_MODE: int(chem_plus_mode.shape[1]),
        },
        "n_train_rows": int(len(inputs.train_df)),
        "n_val_rows": int(len(inputs.val_df)),
    }
    (output_dir / "t1e_summary.json").write_text(json.dumps(payload, indent=2))
    log.info("T1-E complete. outputs at %s", output_dir)
    return payload
