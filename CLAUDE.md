...
| **R-LOSS** | Loss family under ranking (MSE vs pairwise/ListMLE/SoftRank), multihot held constant | **NEXT** (promoted ahead of R2, 2026-06-06). R1 showed the objective mismatch (pointwise MSE for a ranking metric) is the most fundamental lever. Must beat chem-kNN gate (NDCG@5 0.485). |
| **R2** | Fusion topology retest under ranking (multihot held constant) | after R-LOSS. FiLM/gating/gene-reduce. |
| **R1-revisit** | Re-run chemistry arms under the R-LOSS+R2 winner | deferred. Finalizes the encoder once arch+loss improve. |
