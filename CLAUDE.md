...
| **R1** | Representation retest under ranking (chemistry: fingerprints vs multihot) | **CONTRACTS LOCKED; integration pending.** All four R-LOCKs approved (contracts + leaf code + tests), but R1 cannot run until the integration layer is built: (1) eligibility-filter + `w_g` impl (R-LOCK-1 is uncoded), (2) split materializer (R-LOCK-2 is uncoded), (3) training loop wired to `RankingBatch` + Spearman early-stop, (4) run-manifest v2 dispatcher (only v1 validator exists), (5) R1 handler registration. Plus open blockers from the 2026-05-25 audit (train-only `r_replicate`; add strong baselines incl. matrix factorization; compute task-relevant noise floor on real data). |
